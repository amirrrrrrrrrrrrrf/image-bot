...
​
# Try multiple models - first one that works will be used
MODELS = [
    "gemini-2.0-flash",
    ...
    for model in MODELS:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                logger.warning(f"Model {model} returned {r.status_code}: {r.text[:100]}")
        except Exception as e:
            logger.warning(f"Model {model} failed: {e}")
    
    raise Exception("هیچ مدلی در دسترس نیست. API Key رو چک کن.")
​
...
async def btn(u,c):
    ...
    if uid not in imgs: await q.message.reply_text("ابتدا تصویر بفرست!"); return
...
async def txt(u,c):
    uid=u.effective_user.id
    if uid not in imgs: await u.message.reply_text("ابتدا تصویر بفرست!"); return
